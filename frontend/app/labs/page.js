...
import { FlaskConical, Atom, Microscope, Cpu, Terminal, Globe, Code2, ArrowRight, Play } from 'lucide-react';
            ...
            {labs.map((lab, index) => {
              ...
              return (
                <motion.div key={lab.id}
                  ...
                  whileHover={{ y: -8 }}
                >
                  <GlassCard className={`overflow-hidden ${lab.status === 'coming-soon' ? 'opacity-60' : ''}`} hover={lab.status === 'available'}>
                    ...
                    <div className={`relative p-6 bg-gradient-to-br ${lab.color} bg-opacity-10`}>
                      <div className={`absolute inset-0 bg-gradient-to-br ${lab.color} opacity-[0.08]`} />
                      <div className="relative z-10">
                        <div className={`inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-br ${lab.color} text-white shadow-lg mb-4`}>
                          <LabIcon size={28} />
                        </div>
                    ...
                    <div className="p-6">
                      <p className="text-sm text-dark-600 dark:text-dark-300 leading-relaxed mb-4">{lab.description}</p>
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-dark-400 dark:text-dark-500">{lab.count} experiments</span>
                        {lab.status === 'available' ? (
                          <Link href={`/labs/${lab.id}`} className={`flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r ${lab.color} text-white text-sm font-bold shadow-lg hover:shadow-xl transition-all duration-300`}>
                            <Play size={14} />
                            Enter Lab
                          </Link>
                        ) : (
